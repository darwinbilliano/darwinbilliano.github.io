...
function NavLinks({ model }: { model: HeaderModel }) {
...
function Links({ model, toggle }: { model: HeaderModel; toggle: boolean }) {
  const className = classNames(
    "flex max-lg:flex-col items-stretch w-full max-lg:py-4 max-lg:absolute left-0 top-14 bg-inherit border-inherit max-lg:border-b",
    {
  ...
  return (
    <nav className="flex items-center justify-between h-14 px-12 lg:px-32 font-bold bg-inherit border-inherit">
      ...
      <div className="flex items-center bg-inherit border-inherit">
        <Links model={model} toggle={state} />
